...
/**
 * Champs communs aux devis et aux factures : client, chantier, régime de TVA,
 * remise globale et lignes.
 */
abstract class DocumentRequest extends FormRequest
{
    ...
    protected function documentRules(): array
    {
        return [
            ...
            'vat_regime' => ['nullable', Rule::in(['franchise', 'assujetti'])],
    ...
    {
        return [
            'client_id' => 'client', 'title' => 'objet', 'vat_regime' => 'régime de TVA',
    ...
    /**
     * En-tête commun prêt à enregistrer.
     *
     * @param  list<string>  $fields  champs propres au type de document
     * @return array<string, mixed>
     */
    protected function headerAttributes(array $fields): array
    {
        $data = $this->safe()->only(array_merge([
            'client_id', 'worksite_id', 'title', 'vat_regime', 'payment_terms', 'notes', 'internal_notes',
        ], $fields));

        $data['worksite_id'] = ($data['worksite_id'] ?? null) ?: null;
        // Sans choix explicite : régime actuel du document, sinon celui des réglages.
