            ...
            @foreach ($results['quotes'] as $quote)
                <li>
                    <a class="list-item" href="{{ route('quotes.show', $quote) }}">
                        <span class="avatar avatar-muted"><x-icon name="file" /></span>
                        <span class="list-main">
                            <strong>{{ $quote->displayNumber() }} · {{ $quote->client?->displayName() }}</strong>
                            <span class="muted small">{{ $quote->title ?: 'Sans objet' }}</span>
                        </span>
                        <span class="list-meta">@include('quotes._status')</span>
                    </a>
                </li>
            @endforeach
        </ul>
